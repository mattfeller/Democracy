...
text \<open>A step function takes four arguments: the set of active processes, the name of the process that is
...
(* Modified step_func from original Network.thy to depend on the set of procs *)
type_synonym ('proc, 'state, 'msg, 'val) step_func =
  \<open>('proc set) \<Rightarrow> 'proc \<Rightarrow> 'state \<Rightarrow> ('proc, 'msg, 'val) event \<Rightarrow> ('state \<times> ('proc, 'msg) send set)\<close>
...
inductive execute ::
  ...
  \<open>\<lbrakk>execute step init procs events msgs states;
    ...
    step procs proc (states proc) event = (new_state, sent);
...
lemma execute_step:
  ...
  shows \<open>\<exists>msgs states sent new_state.
          ...
          step procs proc (states proc) event = (new_state, sent) \<and>
